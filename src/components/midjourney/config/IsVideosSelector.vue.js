...
var DEFAULT_IS_VIDEOS = false;
exports.default = (0, vue_1.defineComponent)({
    name: 'IsVideosSelector',
    components: {
    ...
    computed: {
        value: {
            get: function () {
                return this.$store.state.midjourney.config.is_videos;
            },
            set: function (val) {
                console.debug('set is_videos', val);
                this.$store.commit('midjourney/setConfig', __assign(__assign({}, this.$store.state.midjourney.config), { is_videos: val }));
            }
        }
    },
    mounted: function () {
        if (this.value === undefined) {
            this.value = DEFAULT_IS_VIDEOS;
        }
    }
});
...
(__VLS_ctx.$t('midjourney.name.isVideos'));
