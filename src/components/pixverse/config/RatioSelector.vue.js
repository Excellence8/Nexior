...
exports.default = (0, vue_1.defineComponent)({
    ...
    computed: {
        ...
        value: {
            get: function () {
                var _a;
                return (_a = this.$store.state.pixverse.config) === null || _a === void 0 ? void 0 : _a.aspect_ratio;
            },
            set: function (val) {
                console.debug('set aspect_ratio', val);
                this.$store.commit('pixverse/setConfig', __assign(__assign({}, this.$store.state.pixverse.config), { aspect_ratio: val }));
            }
        }
    },
    mounted: function () {
        if (!this.value) {
            this.value = constants_1.PIXVERSE_DEFAULT_ASPECT_RATIO;
        }
    }
});
...
(__VLS_ctx.$t('pixverse.name.aspectRatio'));
