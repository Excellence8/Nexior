    ...
    watch: {
        urls: {
            handler: function (val) {
                ...
                this.$store.commit('flux/setConfig', __assign(__assign({}, this.$store.state.flux.config), { image_url: val[0] }));
            }
        }
    },
    methods: {
        onExceed: function () {
            element_plus_1.ElMessage.warning(this.$t('flux.message.uploadStartImageExceed'));
        },
        onError: function () {
            element_plus_1.ElMessage.error(this.$t('flux.message.uploadStartImageError'));
        }
    }
});
...
(__VLS_ctx.$t('flux.button.uploadImageUrl'));
var __VLS_19;
var __VLS_7;
